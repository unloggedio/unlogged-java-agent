...
//                        System.err.println(availableBytes + " bytes yet, sleeping");
                    ...
                    int bytesRead = fileInputStream.read(FILE_BUFFER);
                    if (bytesRead == 0) {
...
//                        System.err.println("Consumed " + bytesConsumed + "/" + bytesRead + ": events = " + eventsRead + " after [" + bytesConsumedTotal + "]");
                    ...
                    if (bytesEnd - bytesStart > 1000) {
                        System.err.println("Consumed [" + bytesConsumed + "] at " + (bytesConsumed / 1024) / ((bytesEnd - bytesStart) / 1000) + " Kb/s");
                    }
...
//                System.err.println("exception " + e.getMessage() + "[" + e.getStackTrace()[0].getFileName() + "]" + e.getStackTrace()[0].getLineNumber());
                ...
                System.err.println("Took " + ((end - start) / (1000)) + " seconds for reading file " + file.getAbsolutePath() + " - " + bytesConsumedTotal / 1000 + "Kb" + " => " + eventId);
                if (file != null) {
                    file.delete();
                }
            }
